...
/**
 * Refuse to write embeddings that would mix providers/dimensions within one
 * corpus (R2.2/2.3). Same-dimension provider mixing corrupts similarity search
 * yet passes the DDL CHECK, so it must be caught at ingest time.
 */
export function assertNoProviderMixing(
	...
	if (existing.provider !== incoming.provider || existing.dim !== incoming.dim) {
		throw new Error(
			`corpus already embedded with ${existing.provider}/${existing.dim}; refusing to mix with ` +
				`${incoming.provider}/${incoming.dim}. Migrate + re-ingest to switch embedding models.`,
		);
	}
}
...
export type EmbedBatch = (
	values: string[],
) => Promise<{ embeddings: number[][]; provider: string; dim: number }>;
...
export async function ingest(corpusPath: string, deps: IngestDeps): Promise<IngestSummary> {
	...
	for (const doc of docs) {
		...
		const { embeddings, provider, dim } = await deps.embed(pieces);
		assertEmbeddingConsistency(embeddings, pieces.length, { provider, dim });
		assertNoProviderMixing(existing, { provider, dim });
...
export function createEmbedder(model: EmbeddingModel, provider: string): EmbedBatch {
	return async (values) => {
		...
		return { embeddings, provider, dim: embeddings[0]?.length ?? 0 };
	};
}
...
export function createDrizzleIngestStore(db: PgDatabase<PgQueryResultHKT>): IngestStore {
	return {
		async getEmbeddingProfile() {
			const rows = await db
				.select({ provider: embedding.provider, dim: embedding.dim })
				.from(embedding)
				.limit(1);
			return rows[0] ?? null;
		},
			...
			return db.transaction(async (tx) => {
				...
				await tx.insert(embedding).values(
					doc.chunks.map((c) => {
						...
						return { chunkId, vector: c.embedding, dim: doc.dim, provider: doc.provider };
					}),
				);
