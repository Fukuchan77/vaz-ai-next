...
_Requirements:_ 1.3, 1.4, 1.5, 2.3, 2.4
...
_Depends:_ Task 2(runbook の選択基準を README 的に参照)
_Requirements:_ 2.1, 2.2

- [ ] 3.1 `unit` ジョブから Security Audit ステップを除去し、独立 `audit` ジョブ
  (checkout → mise-action → pnpm-setup → `pnpm install --frozen-lockfile` →
  `pnpm audit --audit-level=moderate`)を追加する。
- [ ] 3.2 ブランチ保護の required checks に `audit` を追加する(リポジトリ設定。
  実施できない場合は PR 説明に運用者向け手順として明記)。
...
- [ ] 4.4 検証: `NODE_ENV` 未設定および `NODE_ENV=development` のシェルから `mise run build` が
  6 ルートを prerender して成功することを確認する。
...
- [ ] 5.2 (P) `to_token_usage` の `total_tokens` 境界を確定する: Pydantic AI `RunUsage` が
  provider-reported total を持つなら採用、無ければ再計算維持。いずれの場合も docstring に
  決定と理由を明記する。テストの期待値を決定に合わせて確認。
