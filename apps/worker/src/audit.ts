 ...
 * contract; the web path supplies its own (Task 20's `apps/web/src/lib/audit.ts`)
 * against the same contract. The single FIRING POINT is the `@vaz/agents`
 * lifecycle audit hook (Task 20.2), which calls `deps.audit.record(...)` for
 ...
 * edge (Task 13.5) supplies a Postgres-backed store and injects the sink via
 * `buildWorkerDeps({ audit })`; tests inject a fake, so no DB is needed.
 *
 * FAIL-LOUD — deliberately the opposite of the fail-soft events sink (13.3).
 ...
 * point (Task 20.2), which can decide per tool destructiveness (R5.3); the sink
 ...
 * container edge (Task 13.5); the `audit_log` DDL is owned there.
 */
