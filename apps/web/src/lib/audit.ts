...
/**
 * `apps/web` audit DB sink (R5.5).
 *
 * Supplies the web-path implementation of the `AuditSink` contract
 * (`@vaz/schemas/deps`), against which the worker-path sink
 * (`apps/worker/src/audit.ts`) is also built — the single FIRING
 * POINT for both paths is the `@vaz/agents` lifecycle audit hook;
 ...
 * (`createAuditLogStore`) and `createAuditSink` (fail-loud
 * persistence + R4.7-safe failure logging). `apps/web` already
 ...
 * `.../[id]/stream/route.ts` do the same for the durable
