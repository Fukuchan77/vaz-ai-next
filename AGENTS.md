...
- **Dep graph direction** — `@vaz/schemas` and `@vaz/db` are the leaves (no runtime @vaz imports); `@vaz/config`, `@vaz/tools`, `@vaz/rag` depend on the leaves; `@vaz/agents` depends on all of those; `apps/web` and `apps/worker` depend on all. Never invert this.
...
- **Embedding dimension is DDL-fixed at 768** (`EMBEDDING_DIM` in `packages/db/src/schema.ts`). Changing providers with a different dimension requires a migration + full re-ingest, never a runtime change.
