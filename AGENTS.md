...
- `pnpm -r run typecheck` — type check across the whole workspace (there is no root `tsconfig.json`, so a bare `pnpm exec tsc --noEmit` from the repo root fails with `TS5081`; `apps/web`/`apps/worker` each have their own and can be typechecked directly with `pnpm --filter @vaz/web exec tsc --noEmit`, or `cd`'d into and run bare)
...
- **Approvals must be SIGNED or the gate is decorative (R5.6)** — a `tool-approval-response` is client-supplied data, and `convertToModelMessages` rebuilds the matching `tool-approval-request` part *out of that same client message*. So the SDK's `InvalidToolApprovalError` ("no matching tool-approval-request") can never fire for a forged payload — the fake pair always agrees with itself. The only real check is the HMAC signature the SDK adds to each request and verifies on the response, and that entire path is **skipped** unless `streamText` receives a signing key. `@vaz/agents`' `buildStreamTextOptions` resolves one via `resolveApprovalSigningKeyStatus()` (in `packages/agents/src/approval-signing.ts` — `TOOL_APPROVAL_SECRET` takes priority and is used verbatim; otherwise a usable `AUTH_SECRET` is run through HKDF-SHA256 with a fixed, feature-specific `info` string before use, so the derived signing key and Auth.js's own session-encryption key stay cryptographically separate even though both come from the same variable; `{ key: undefined, reason }` if neither is usable) and passes the resolved key as **`experimental_toolApprovalSecret`** — the SDK's spelling as of ai@7.0.97. Getting that name wrong is SILENT (`streamText` destructures known options and drops the rest; the options object is built separately so excess-property checking never sees it), which is why `packages/agents/tests/chat-agent.spec.ts` drives a forged approval end-to-end instead of asserting the option is present. When no key resolves, `createToolApprovalPolicy({ approvalsAreVerifiable: false })` returns `'denied'` instead of `'user-approval'` — fail closed, because asking a human is pointless if the answer cannot be authenticated. `playwright.config.ts` pins a test key in `webServer.env` so `hitl-approval.spec.ts` verifies the real property rather than passing because a model call failed for lack of credentials.
- **`TOOL_APPROVAL_SECRET` minimum 32 chars, and so is the `AUTH_SECRET` fallback** — the dedicated key is enforced by Zod schema (`z.string().min(MIN_APPROVAL_SIGNING_KEY_LENGTH)`, currently 32) and fails loudly (throws) if set-but-too-short, because a short HMAC key is brute-forceable offline from a single observed signature. `AUTH_SECRET` has no length floor of its own (Auth.js doesn't enforce one), so `resolveApprovalSigningKeyStatus` applies the same floor to it — but fails soft (`reason: "auth-secret-too-short"`, not a throw), since it's a pre-existing Auth.js variable and a chat request must not crash over a length rule this module imposes on someone else's setting. `buildStreamTextOptions` logs a `logger.warn` naming the cause and passes a specific denial reason (`AUTH_SECRET_TOO_SHORT_DENIAL_REASON`, not the generic `UNVERIFIABLE_APPROVAL_DENIAL_REASON`) to the policy for this case, so an operator who set `AUTH_SECRET` expecting it to cover approval signing too isn't left thinking nothing is configured. `emptyToUndefined` strips blank values so `KEY=` in a dotenv file is treated as unset, not a too-short key.
