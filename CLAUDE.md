...
- **Monorepo**: two apps (`apps/web` = `@vaz/web`, `apps/worker` = `@vaz/worker`) over seven source-only `@vaz/*` packages (`schemas`, `db`, `config`, `tools`, `rag`, `agents`, `evals`). One-way dep graph: `schemas`/`db` (leaves) → `config`/`tools`/`rag` → `agents` → `web`/`worker`/`evals`. Packages ship raw TS (no build step, no per-package `tsconfig`); import by path (`@vaz/schemas/deps`). The legacy root `src/`/`tests/` migration (spec `001-vaz-ai-update`) is complete — `apps/web` is the sole source of truth; no application source lives at the repo root.
